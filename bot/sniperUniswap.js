...
const TokenContract = process.env.TokenContract,
  ...
  amountIn = ethers.utils.parseUnits(process.env.DAI_To_Use, "ether"),
...
const WETH = "0xc778417E063141139Fce010982780140Aa0cD5Ab",
  uniRouterV2Addr = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D", 
  factoryRouter = "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f ",
...
async function getWETHTokenBalance(e, o, n) {
  const t = new ethers.Contract(
    e,
    [
      {
...
const uniRouterV2B = new ethers.Contract(
    uniRouterV2Addr,
    [
  ...
  console.log(chalk.red("Monitoring for rug pull in progress ....\n")),
    provider.on("pending", async (n) => {
      ...
      if (
        ...
        const a =
          await uniRouterV2.swapExactTokensForETHSupportingFeeOnTransferTokens(
            e,
            amountOutMin,
            [tokenAddress, WETH],
            myAddress,
...
if (
  ...
  console.log(
    chalk.black.bgRed(
      "We are not responsabile if you bought this from a reseller and not directly from us at https://t.me/ultimabotpro. Doing it will result in no updates and no support if the bot is not operative anymore. \n"
    )
  ),
  2 == choseMode)
)
  (detectDxSale && detectFairLaunch) || (!detectDxSale && !detectFairLaunch)
    ...
    ? (async () => {
        ...
        let o = await getWETHTokenBalance(WETH, myAddress, provider);
        if (
          (console.log(
            chalk.cyan(
          ...
          console.log(
            chalk.green(
              `Total WETH balance is ${chalk.yellow(
        ...
        if (buyOnly) {
          ...
          if (1 == antiBotMultiTx && 0 == passed) {
            for (i = 0; i < txNumberForAntibot - 1; i++) {
              ...
              await uniRouterV2B.swapExactTokensForTokens(
                amountIn,
                amountOutMin,
                [WETH, tokenAddress],
                myAddress,
            ...
            const e = await uniRouterV2B.swapExactTokensForTokens(
              amountIn,
              amountOutMin,
              [WETH, tokenAddress],
              myAddress,
          ...
          (pairDetect = await e.getPair(WETH, tokenAddress)),
    ...
    : detectFairLaunch &&
      (async () => {
        currentNonce = await getNonce(myAddress);
        let e = await getWETHTokenBalance(WETH, myAddress, provider);
        if (
          (console.log(
            chalk.cyan(
              "For Support contact developer on https://t.me/ultimabotpro. \n"
              ...
              if (
                ...
                if (isLiqudityInRange(o, expected)) {
                  ...
                  if (1 == antiBotMultiTx && 0 == passed) {
                    ...
                    const o = await uniRouterV2B.swapExactTokensForTokens(
                      amountIn,
                      amountOutMin,
                      [WETH, tokenAddress],
                      myAddress,
          ...
          provider.on("pending", async (e) => {
            ...
            if (
              ...
              const e = o.gasLimit.mul(2),
                n = o.gasPrice.mul(2);
              console.log(chalk.green("Start buying token..."));
              const t = await uniRouterV2B.swapExactTokensForTokens(
                amountIn,
                amountOutMin,
                [WETH, tokenAddress],
                myAddress,
